...
const fetchTags = async (aid: string, cid: string) => {
  ...
  return (tags as { tag_name?: string }[])
    .map(tag => tag.tag_name?.trim())
    .filter((tag): tag is string => Boolean(tag))
}

const getEpisodePages = (episode?: { page?: { cid?: number }; pages?: Array<{ cid?: number }> }) => {
  if (!episode) {
    return []
  }
...
const findTargetEpisode = (info: VideoInfo, cid: string) => {
  ...
  return (
    episodes.find(episode => getEpisodePages(episode).some(page => String(page.cid) === String(cid))) ??
  ...
  if (!videoContextCache.has(key)) {
    videoContextCache.set(
      ...
      (async () => {
        ...
        const tags = await fetchTags(aid, String(cid))
